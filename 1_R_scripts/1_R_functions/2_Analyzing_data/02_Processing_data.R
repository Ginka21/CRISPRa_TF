...
NormalizeWithNTControls <- function(use_df, norm_method = "all NT") {
  ...
  has_Glo <- "CellTiterGlo_raw" %in% names(use_df)

  if (has_Glo) {
    use_df[, "CellTiterGlo_foldNT"] <- NormPlates(use_df, "CellTiterGlo_raw", foldNT = TRUE, norm_method = norm_method)
    after_column <- "CellTiterGlo_raw"
  } else {
    after_column <- "Is_pos_ctrl"
  }

  are_after <- seq_len(ncol(use_df)) > which(names(use_df) == after_column)
  use_columns <- unique(c(names(use_df)[!(are_after)], after_column,
